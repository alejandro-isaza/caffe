...
namespace caffe {
    ...
    TriSliceDataLayer<Dtype>::~TriSliceDataLayer<Dtype>() {
        this->JoinPrefetchThread();
    }

    template <typename Dtype>
    void TriSliceDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
                                               const vector<Blob<Dtype>*>& top) {
        string root_folder = this->layer_param_.tri_slice_data_param().root_folder();

        // Read the file with filenames and labels
        const string& source = this->layer_param_.tri_slice_data_param().source();
    ...
    template <typename Dtype>
    void TriSliceDataLayer<Dtype>::ShuffleFiles() {
    ...
    template <typename Dtype>
    void TriSliceDataLayer<Dtype>::InternalThreadEntry() {
        ...
        TriSliceDataParameter tri_slice_data_param = this->layer_param_.tri_slice_data_param();
        const int batch_size = tri_slice_data_param.batch_size();
        string root_folder = tri_slice_data_param.root_folder();
        const auto sample_count = this->layer_param_.tri_slice_data_param().sample_count();
        ...
        for (int item_id = 0; item_id < batch_size; ++item_id) {
            ...
            fetchFFTransformedData(root_folder + fileNames[0], data + (0 * sample_count), offsets[0], gain, sample_count);
            fetchFFTransformedData(root_folder + fileNames[1], data + (1 * sample_count), offsets[1], gain, sample_count);
            fetchFFTransformedData(root_folder + fileNames[2], data + (2 * sample_count), offsets[2], gain, sample_count);
